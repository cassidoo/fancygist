		...
		<AnimatePresence>
			{show && (
				<div className="slash-command-menu-positioner fixed w-60 max-w-fit h-min [perspective:600px]">
					<motion.div
						ref={menuRef}
						className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-lg py-2 max-h-64 max-w-64 overflow-y-auto z-50"
						...
						{commands.map((command, index) => (
							<button
								...
								className={`w-full text-left px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors cursor-pointer ${
									index === selectedIndex
										? "bg-gray-50 dark:bg-gray-700/50 border-l-2 border-lime-600"
